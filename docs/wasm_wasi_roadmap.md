...
- Component decoding and validation exist under
  ...
  plus record/tuple/fixed-list/flags/enum subset through flat scalar values
  such as canonical string `(ptr, len)` pairs, field-by-field record scalars,
  flags bitsets, and enum discriminants. These are value-codec adapter
  boundaries, not a complete flattened core function ABI. Async value
...
- Component resource table canonical `resource.new`/`resource.rep`/
  ...
  adapter flat-scalar invocation, decoded direct string adapter memory

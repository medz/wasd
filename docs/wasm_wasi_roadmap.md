...
- [x] `P1-NODE-HOST-FS-READ-WRITE` - Node.js Preview1 preopens are backed by
  ...
  - Scope: Node.js Preview1 `path_open`, `fd_read`, `fd_pread`, `fd_write`,
    `fd_pwrite`, `fd_seek`, `fd_tell`, `fd_filestat_get`,
    `fd_filestat_set_size`, `fd_allocate`, `O_TRUNC`, and `O_CREAT` for regular
    files below configured preopen directories. Browser JS remains virtual;
    native VM already has a broader real-host-filesystem bridge.
  - Edit targets: `lib/src/wasi/preview1/js/web/wasi.dart`,
  ...
  - Red test:
    ...
    failed before the fix with `path_open` returning `NOENT(44)` because Node
    preopens were satisfied by the in-memory VFS instead of live host file
  ...
  - Implementation gate:
    ...
    `dart analyze test/wasi_test.dart lib/src/wasi/preview1/js/web/wasi.dart test/support/node_host_fs.dart test/support/node_host_fs_stub.dart test/support/node_host_fs_js.dart`.
  - Performance gate:
    `dart run tool/wasi_vfs_benchmark.dart --distribution=all --json`
    reported baseline `path_open_close.per_operation_us=1.2045`,
    `file_fd_read_write.per_operation_us=0.2915`, and
    `socket_poll_readiness.per_operation_us=0.4244545454545455`; keep
    `P1-NODE-HOST-FS-BENCHMARK` as the next required row before expanding
    beyond regular files into directory traversal/mutation work.
  ...
  - Claim impact: removes the Node memory-only regular-file blocker for
    `SUPPORT-P1`; directory enumeration, path mutation, links, symlinks,
    timestamps, and official-suite evidence remain separate P1 work.
- [x] `P1-NODE-HOST-FS-BENCHMARK` - Add a Node.js host-regular-file benchmark
  before expanding the host filesystem bridge beyond regular files.
  - Scope: dart2js/Node benchmark coverage for Node host preopen regular-file
    `path_open`, positioned read/write, create, truncate, set-size, allocate,
    and close cycles, with a comparable virtual-file baseline.
  - Edit targets: `tool/wasi_node_host_fs_benchmark.dart`,
    `tool/wasi_node_host_fs_benchmark_js.dart`,
    `tool/run_wasi_node_host_fs_benchmark.mjs`, `.dart_tool/` generated output
    only at runtime, and roadmap evidence wording.
  - Red test: N/A; this is a performance evidence row. The benchmark must fail
    non-zero if Node is unavailable or if any host readback/resize assertion
    fails.
  - Implementation gate:
    `dart analyze tool/wasi_node_host_fs_benchmark.dart tool/wasi_node_host_fs_benchmark_js.dart`;
    `dart run tool/wasi_node_host_fs_benchmark.dart --iterations=25 --warmup=5 --payload-bytes=256 --json`;
  ...
  - Done when: Node host-file numbers are reproducible locally, no benchmark
    path snapshots host files into memory, and future Node directory/mutation
    work has an explicit comparison point.
...
- [x] `P1-NODE-HOST-FS-DIRECTORY-READDIR` - Node.js Preview1 preopened host
  ...
  - Scope: Node.js Preview1 `path_open(O_DIRECTORY)`, directory descriptor
    ...
    behavior for directories below configured preopen directories.
  - Edit targets: `lib/src/wasi/preview1/js/web/wasi.dart`,
    `test/wasi_test.dart`, `test/support/node_host_fs*.dart`, README support
    wording, and this roadmap.
  - Red test:
  ...
  - Evidence update: this board, `P1-FULL-RUNTIME-GATE`, verification matrix,
    `SUPPORT-P1`, and README support wording.
  - Claim impact: removes the Node host-directory enumeration blocker for
    `SUPPORT-P1`; path mutation, links, symlinks, timestamps, and official suite
    evidence remain separate P1 work.
- [x] `P1-NODE-HOST-FS-BASIC-PATH-MUTATION` - Node.js Preview1 path mutation
  writes to configured real host preopen directories instead of shadowing changes
  in the portable in-memory VFS.
  - Scope: Node.js Preview1 `path_create_directory`, `path_unlink_file`,
    `path_remove_directory`, and `path_rename` below configured preopen
    directories. Browser JS remains virtual; native VM already has broader host
    path mutation including hard links and symlinks.
  - Edit targets: `lib/src/wasi/preview1/js/web/wasi.dart`,
    `test/wasi_test.dart`, `test/support/node_host_fs*.dart`, README support
    wording, and this roadmap.
  - Red test:
    `dart test -p node test/wasi_test.dart --name "node preview1 mutates real host directory paths" --reporter=expanded`
    failed before the fix because `path_create_directory` returned success while
    the host temp directory did not contain the created directory.
  - Implementation gate:
    `dart test -p node test/wasi_test.dart --name "node preview1 mutates real host directory paths" --reporter=expanded`;
    Node host regular-file and directory focused tests; VM/browser Preview1
    regression suites; README command/snippet tests; `dart analyze`.
  - Performance gate:
    no recursive traversal, broad host scan, or preopen-tree snapshotting in this
    row. Host operations use direct Node `fs` calls after a single resolved
    preopen-root check and one-level parent/type checks needed for Preview1 errno
    fidelity.
  - Done when: `path_create_directory` creates a real host directory,
    `path_unlink_file` deletes a real host file while preserving trailing-slash
    errors, `path_remove_directory` removes only empty real host directories,
    `path_rename` renames real host files/directories and rejects non-empty
    directory replacement, and virtual paths still fall back to the in-memory VFS.
  - Evidence update: this board, `P1-FULL-RUNTIME-GATE`, verification matrix,
    `SUPPORT-P1`, and README support wording.
  - Claim impact: removes the Node host path-mutation blocker for `SUPPORT-P1`;
    the later `P1-NODE-HOST-FS-LINK-SYMLINK` row covers hard links and symlink
    create/readlink, and `P1-NODE-HOST-FS-SYMLINK-RESOLUTION` covers
    follow/path metadata; timestamp mutation, full lstat-grade symlink
    timestamp/set-times metadata, and final support-gate evidence remain
    separate P1 work.
- [x] `P1-NODE-HOST-FS-LINK-SYMLINK` - Node.js Preview1 host preopens support
  real hard-link creation plus symlink creation/readlink.
  - Scope: Node.js Preview1 `path_link` for regular host files and
    `path_symlink`/`path_readlink` below configured preopen directories. Browser
    JS remains virtual; native VM already has broader host symlink resolution.
  - Edit targets: `lib/src/wasi/preview1/js/web/wasi.dart`,
    `test/wasi_test.dart`, `test/support/node_host_fs*.dart`, README support
    wording, and this roadmap.
  - Red test:
    `dart test -p node test/wasi_test.dart --name "node preview1 creates real host links and symlinks" --reporter=expanded`
    failed before the fix with `path_link` returning `NOENT(44)` for a real host
    preopen file because the syscall only checked the in-memory VFS.
  - Implementation gate:
    `dart test -p node test/wasi_test.dart --name "node preview1 creates real host links and symlinks" --reporter=expanded`;
    Node host filesystem focused tests; VM/browser Preview1 regression suites;
    README command/snippet tests; `dart analyze`.
  - Performance gate:
    no recursive traversal, broad host scan, or preopen-tree snapshotting in this
    row. Each syscall performs direct Node `fs` work after a resolved preopen-root
    check and one-level type/parent checks for Preview1 errno fidelity.
  - Done when: `path_link` creates a real hard link for a regular host file,
    `path_symlink` creates a real host symlink with the guest target string,
    `path_readlink` returns that target through guest memory, directory hard
    links are rejected, absolute symlink targets return `NOTCAPABLE`, and virtual
    paths still fall back to the in-memory VFS.
  - Evidence update: this board, `P1-FULL-RUNTIME-GATE`, verification matrix,
    `SUPPORT-P1`, and README support wording.
  - Claim impact: removes the Node host hard-link and symlink create/readlink
    blocker for `SUPPORT-P1`; `P1-NODE-HOST-FS-SYMLINK-RESOLUTION` covers
    follow/path metadata, while timestamp mutation, full lstat-grade symlink
    timestamp/set-times metadata, and final support-gate evidence remain
    separate P1 work.
- [x] `P1-NODE-HOST-FS-SYMLINK-RESOLUTION` - Node.js Preview1 host preopens
  follow real host symlinks for open and path metadata.
  - Scope: Node.js Preview1 `path_open` and `path_filestat_get` for final
    symlink targets below configured host preopens. Browser JS remains virtual;
    native VM has the corresponding host symlink row checked separately.
  - Edit targets: `lib/src/wasi/preview1/js/web/wasi.dart`,
    `test/wasi_test.dart`, `test/support/node_host_fs*.dart`, README support
    wording, and this roadmap.
  - Red test:
    `dart test -p node test/wasi_test.dart --name "node preview1 follows real host symlinks and reports path filestat" --reporter=expanded`
    failed before the fix with `path_filestat_get` returning `NOENT(44)` for a
    real host symlink because the Node host path metadata path only queried the
    in-memory VFS.
  - Implementation gate:
    `dart test -p node test/wasi_test.dart --name "node preview1 follows real host symlinks and reports path filestat" --reporter=expanded`;
    `dart test -p node test/wasi_test.dart --name "node preview1 (preopens use real host regular files|fd_readdir lists real host directories|mutates real host directory paths|creates real host links and symlinks|follows real host symlinks and reports path filestat)" --reporter=expanded`.
  - Performance gate: no recursive traversal, broad host scan, or preopen-tree
    snapshotting in this row. Node host symlink resolution uses direct
    `lstatSync`/`readlinkSync`/`realpathSync` calls on the addressed path and
    checks the resolved target against the resolved preopen root.
  - Done when: no-follow `path_open` on a real host symlink reports `LOOP`;
    follow `path_open` can read a symlinked host file and open a symlinked host
    directory; `path_filestat_get` reports symlink type/target-size without
    follow and target file/directory type/size with follow; dangling symlinks
    report `NOENT`; self-loop symlinks report `LOOP`; and resolved targets
    outside the preopen remain capability failures.
  - Evidence update: this board, `P1-FULL-RUNTIME-GATE`, verification matrix,
    `SUPPORT-P1`, and README support wording.
  - Claim impact: removes the Node host symlink-follow and host path-filestat
    blocker for `SUPPORT-P1`; Node host timestamp mutation, full lstat-grade
    symlink timestamp/set-times metadata, and final support-gate evidence remain
    separate P1 work.
...
- [ ] `P1-FULL-RUNTIME-GATE` - Finish package-level Preview1 support across VM,
  ...
  - Checked child rows: `P1-JS-NODE-SHARED-HOST`,
    ...
    `P1-NODE-HOST-FS-BASIC-PATH-MUTATION`,
    `P1-NODE-HOST-FS-LINK-SYMLINK`,
    `P1-NODE-HOST-FS-SYMLINK-RESOLUTION`,
...
- [ ] `WIT-INGESTION` - Bind imported/generated WIT worlds through versioned
  ...
  - Checked child rows: `WIT-WORLD-VERSION-PROFILE-INGESTION` covers annotated
    ...
    adapter signatures; `WIT-WORLD-RESOURCE-ADAPTER-BINDING` covers local WIT
    `resource` declarations referenced by owned and borrowed synchronous
    adapter handle signatures.
...
- [x] `P1-NODE-HOST-FS-SYMLINK-RESOLUTION` - Node.js Preview1 host preopens
  follow real host symlinks for open and path metadata.
  - Scope: Node.js Preview1 `path_open` and `path_filestat_get` final-symlink
    behavior below configured host preopens.
  - Edit targets: `lib/src/wasi/preview1/js/web/wasi.dart`,
    `test/wasi_test.dart`, `test/support/node_host_fs*.dart`, README support
    wording, and this roadmap.
  - Red test:
  ...
  - Implementation gate:
    ...
    `dart test -p node test/wasi_test.dart --name "node preview1 (preopens use real host regular files|fd_readdir lists real host directories|mutates real host directory paths|creates real host links and symlinks|follows real host symlinks and reports path filestat)" --reporter=expanded`.
  - Performance gate: no recursive traversal, broad host scan, or preopen-tree
    snapshotting; the implementation uses direct Node `fs` path calls and
    resolved-root capability checks.
  - Done when: no-follow host symlinks return `LOOP`; follow opens and reports
    symlinked host files/directories; dangling links return `NOENT`; self-loops
    return `LOOP`; and resolved targets outside the preopen return
    `NOTCAPABLE`.
  - Evidence update: current execution board, `P1-FULL-RUNTIME-GATE`, support
    gate, verification matrix, current baseline, README support wording, and
    this recently checked entry.
  - Claim impact: removes the Node host symlink-follow/path-filestat blocker for
    `SUPPORT-P1`; it does not complete full Preview1, Preview2, or Preview3.
...
- [x] `CORE-ALL-TARGET-RUNNER-GATE` - Core Wasm spec gate runs reliably across
  ...
  - Scope: `tool/spec_runner.dart` all-target core gate orchestration, not a
    new Wasm opcode or public support claim. The row covers local regression
    selection for VM vs JS, stale result-file protection, and the wasm-compiled
    spec player Node stack budget.
  - Edit targets: `tool/spec_runner.dart` and this roadmap.
  - Red test:
    `dart run tool/spec_runner.dart --target=all --suite=core --json=.dart_tool/spec_runner/spec_runner_all_core_current_20260629.json --report=.dart_tool/spec_runner/spec_runner_all_core_current_20260629.md`
    failed before the fix because `js:js-tests` ran VM-only tests that call
    `dart:io` process/filesystem APIs under dart2js/node, and
    `wasm:core-player-wasm-run` failed with `RangeError: Maximum call stack size
    exceeded`. The failure also exposed that the report step could read stale
    result JSON after a player failure.
  - Implementation gate:
    `dart analyze tool/spec_runner.dart`;
    `dart test -p node test/component_test.dart test/readme_snippets_test.dart test/wasm_memory_test.dart test/wasm_predecode_test.dart test/wasm_table_test.dart test/wasm_test.dart --reporter=compact`;
    `dart run tool/spec_runner.dart --target=js --suite=core --no-component-subset --no-component-official --json=.dart_tool/spec_runner/spec_runner_js_core_after_runner_fix.json --report=.dart_tool/spec_runner/spec_runner_js_core_after_runner_fix.md`;
    `dart run tool/spec_runner.dart --target=wasm --suite=core --no-component-subset --no-component-official --json=.dart_tool/spec_runner/spec_runner_wasm_core_after_runner_fix.json --report=.dart_tool/spec_runner/spec_runner_wasm_core_after_runner_fix.md`;
    `dart run tool/spec_runner.dart --target=all --suite=core --json=.dart_tool/spec_runner/spec_runner_all_core_after_runner_fix_20260629.json --report=.dart_tool/spec_runner/spec_runner_all_core_after_runner_fix_20260629.md`.
  - Performance gate:
    `dart run tool/doom_instantiate_profile.dart --compile-breakdown --json`
    reported `total_duration_ms=441` and `peak_rss_bytes=412581888`;
    `dart run tool/doom_instantiate_profile.dart --instantiate-breakdown --json`
    reported `total_duration_ms=660` and `peak_rss_bytes=685211648`.
  - Done when: JS target local tests run only the portable component/readme
    snippet/core Wasm subset, player result JSON is deleted before JS/wasm
    player runs so stale reports cannot mask failures, wasm player runs Node
    with `--stack-size=8192`, VM/JS/wasm core suite totals all report
    `65205/65205` commands passed with zero skipped, and cross-target
    consistency passes.
  - Evidence update: this checked row, the `CORE-OFFICIAL-SPEC-COMPLETE`
    checked-child list, `SUPPORT-WASM`, the verification matrix, and current
    baseline.
  - Claim impact: proves the current vendored core spec suite gate across
...
- [x] `P1-NATIVE-HOST-FS-SYMLINK-READLINK` - Native Preview1
  ...
  - Edit targets: `lib/src/wasi/preview1/native/wasi.dart`,
    `test/wasi_native_host_fs_test.dart`, `README.md`, and this roadmap.
  - Red test:
    `dart test test/wasi_native_host_fs_test.dart --name "native host path_symlink creates real symbolic links" --reporter=compact`
    failed before the fix because `path_symlink` returned success while no real
    host symlink existed.
  - Implementation gate:
    `dart test test/wasi_native_host_fs_test.dart --reporter=compact`;
    `dart test test/wasi_test.dart --reporter=compact`;
    `dart test -p node test/wasi_test.dart --reporter=compact`;
  ...
  - Performance gate:
    `dart run tool/wasi_vfs_benchmark.dart --distribution=all --json`;
    host symlink create/readlink use direct `dart:io` filesystem calls, add no
    recursive scan, and keep the shared VFS hot paths unchanged.
  - Done when: `path_symlink` creates a real host symlink with the guest
    relative target string, `path_readlink` returns that target through guest
    memory, `path_unlink_file` removes the real symlink without deleting the
    target, existing link paths return `EXIST`, readlink on a regular host file
    returns `INVAL`, absolute symlink targets return `NOTCAPABLE`, and missing
...
- [x] `P1-NATIVE-HOST-FS-RENAME` - Native Preview1 `path_rename` mutates real
  ...
  - Scope: Dart VM native Preview1 `path_rename` for regular files and
    directories below configured real host preopen roots. Browser and Node JS
    remain on the in-repo in-memory VFS.
  - Edit targets: `lib/src/wasi/preview1/native/wasi.dart`,
    `test/wasi_native_host_fs_test.dart`, `README.md`, and this roadmap.
  - Red test:
    `dart test test/wasi_native_host_fs_test.dart --reporter=compact`
    failed before the fix because renaming a real host `source.txt` returned
    `NOENT` through the in-memory VFS path.
  - Implementation gate:
    `dart test test/wasi_native_host_fs_test.dart --reporter=compact`;
    `dart test test/wasi_test.dart --reporter=compact`;
  ...
  - Performance gate:
    `dart run tool/wasi_vfs_benchmark.dart --distribution=all --json`;
    no recursive traversal or broad host scan in this row. Add host rename
    benchmark evidence before cross-preopen, batch, or high-volume rename
    support.
  - Done when: host file rename moves bytes to the new path, host file rename
    can replace an existing file, host directory rename moves child entries,
    host directory rename can replace an empty target directory, non-empty
    target directories return `NOTEMPTY`, missing sources return `NOENT`, and
...
- [x] `WIT-WORLD-RESOURCE-ADAPTER-BINDING` - Execute local WIT resource handles
  ...
  - Scope: local WIT interfaces imported by a selected world, limited to
    synchronous signatures that reference `resource` declarations in the same
    interface as owned handles spelled by the resource name or borrowed
    `borrow<T>` handles represented by canonical `u32`
    handles.
  - Edit targets: `lib/src/wasi/component/wit_document.dart`,
  ...
  - Red test:
    `dart test test/wasi_component_wit_test.dart --name "resource type" --reporter=expanded`
    failed before the fix because parsed interfaces exposed no resource
    declarations;
    `dart test test/wasi_component_versioned_host_test.dart --name "resource handles" --reporter=expanded`
    failed before the fix because WIT signatures containing `descriptor` and
    `borrow<descriptor>` were unsupported and `canBindAdapters` stayed false.
  - Implementation gate:
    `dart test test/wasi_component_wit_test.dart --name "resource type" --reporter=expanded`;
    `dart test test/wasi_component_versioned_host_test.dart --name "resource handles" --reporter=expanded`;
  ...
  - Performance gate:
    `dart run tool/wasi_resource_table_benchmark.dart --iterations=2000 --resources=256 --json`
    reported `component_wit_resource_adapter_program_invoke.operations=6000`
    and
    `component_wit_resource_adapter_program_invoke.per_operation_us=0.23233333333333334`.
  - Done when: parsed local WIT `resource` declarations are retained on their
    interface, same-interface resource names resolve during adapter signature
    parsing, owned and borrowed resource handle values execute through
    Preview2 and Preview3 world adapter plans, invalid handle values fail
    before callbacks run, callback results are validated before returning, and
    resource lifetime/drop semantics remain owned by the component resource
    host instead of being faked by WIT adapter glue.
  ...
  - Claim impact: advances `WIT-INGESTION` and concrete `SUPPORT-P2`/`SUPPORT-P3`
    ...
    cross-interface named type resolution, resource lifetime/drop integration,
...
- [ ] `SUPPORT-P1` - Full WASI Preview1 support.
  - Current: `WASI(...)` provides a real Preview1 host surface. Native and JS
    ...
    plus real host path mutation for create-directory, unlink-file,
    remove-directory, rename, hard-link, symlink-create, and symlink-readlink
    flows, plus real host symlink follow/path-metadata for final symlink targets
    that resolve inside the configured preopen. Node host timestamp mutation,
    full lstat-grade symlink timestamp/set-times metadata, and official
    support-gate evidence remain incomplete. Node host-file and directory
    performance evidence now exists through
    ...
    Preview1 support claim is reconciled across VM, browser, Node, README/API
    wording, and any non-testsuite Preview1 gaps.
  - Required rows: every Preview1 descriptor, filesystem, stdio, clock, random,
    ...
    `P1-OFFICIAL-FS-CONFORMANCE` child rows, `P1-JS-NODE-SHARED-HOST`,
    `P1-NODE-HOST-FS-READ-WRITE`, `P1-NODE-HOST-FS-BENCHMARK`,
    `P1-NODE-HOST-FS-DIRECTORY-READDIR`, and
    `P1-NODE-HOST-FS-BASIC-PATH-MUTATION`, and
    `P1-NODE-HOST-FS-LINK-SYMLINK`,
    `P1-NODE-HOST-FS-SYMLINK-RESOLUTION`, plus future official testsuite gap
...
- [ ] `SUPPORT-P2` - Full WASI 0.2 / Preview2 support.
  - Current: the public factory rejects Preview2; internal versioned component
    ...
    `list`/`tuple`, named record, variant/enum/flags, and resource-handle WIT
    world adapters exist.
...
- [ ] `SUPPORT-P3` - Full WASI 0.3 / Preview3 support.
  - Current: the public factory rejects Preview3; internal P3 resources, async
    ...
    `option`/`result`, `list`/`tuple`, named record, variant/enum/flags, and
    resource-handle WIT world adapters now execute through the Preview3
    versioned host.
...
| [x] | Preview1 Node.js host regular-file preopen backing | `lib/src/wasi/preview1/js/web/wasi.dart`, `test/wasi_test.dart`, `test/support/node_host_fs*.dart`, `README.md` | `dart test -p node test/wasi_test.dart --name "node preview1 preopens use real host regular files" --reporter=expanded`; `dart test -p node test/wasi_test.dart --reporter=compact`; `dart test test/wasi_test.dart --reporter=compact`; `dart test -p chrome test/wasi_test.dart --reporter=compact`; `dart test test/readme_snippets_test.dart test/readme_commands_test.dart --reporter=compact`; `dart analyze test/wasi_test.dart lib/src/wasi/preview1/js/web/wasi.dart test/support/node_host_fs.dart test/support/node_host_fs_stub.dart test/support/node_host_fs_js.dart`; `dart run tool/wasi_vfs_benchmark.dart --distribution=all --json` | Node preopened regular files now use live host filesystem descriptors for read, positioned read/write, create, truncate, `fd_filestat_set_size`, and `fd_allocate` without snapshotting files into the VFS. Node host timestamp mutation, full lstat-grade symlink timestamp/set-times metadata, and official support-gate evidence remain incomplete, so `SUPPORT-P1` stays unchecked. |
| [x] | Preview1 Node.js host directory readdir backing | `lib/src/wasi/preview1/js/web/wasi.dart`, `test/wasi_test.dart`, `test/support/node_host_fs*.dart`, `tool/wasi_node_host_fs_benchmark_js.dart`, `README.md` | `dart test -p node test/wasi_test.dart --name "node preview1 fd_readdir lists real host directories" --reporter=expanded`; `dart analyze lib/src/wasi/preview1/js/web/wasi.dart test/wasi_test.dart test/support/node_host_fs_js.dart test/support/node_host_fs_stub.dart tool/wasi_node_host_fs_benchmark.dart tool/wasi_node_host_fs_benchmark_js.dart`; `dart run tool/wasi_node_host_fs_benchmark.dart --iterations=25 --warmup=5 --payload-bytes=256 --json`; `dart run tool/wasi_node_host_fs_benchmark.dart --json` | Node preopen fd 3 now reads live host root directory entries, and `path_open(O_DIRECTORY)` opens a host directory descriptor whose `fd_readdir` reports deterministic one-level host entries without snapshotting the tree at `WASI` construction. Default benchmark reported host `497.26us` preopen readdir and `43.166us` directory open/readdir/close with host readback assertions passing. Node host timestamp mutation, full lstat-grade symlink timestamp/set-times metadata, and official support-gate evidence remain incomplete. |
| [x] | Preview1 Node.js host path mutation backing | `lib/src/wasi/preview1/js/web/wasi.dart`, `test/wasi_test.dart`, `test/support/node_host_fs*.dart`, `README.md` | `dart test -p node test/wasi_test.dart --name "node preview1 mutates real host directory paths" --reporter=expanded` failed before the fix because the host temp directory did not contain the created directory; the same command passes after the fix. Full verification for this row: `dart test -p node test/wasi_test.dart --name "node preview1 mutates real host directory paths" --reporter=expanded`; Node host regular-file and directory focused tests; VM/browser Preview1 regression suites; README command/snippet tests; `dart analyze`. | Node configured host preopens now execute `path_create_directory`, `path_unlink_file`, `path_remove_directory`, and `path_rename` through direct Node `fs` calls after preopen-root resolution and one-level type checks. Virtual files and browser JS remain on the in-memory VFS. Node host timestamp mutation, full lstat-grade symlink timestamp/set-times metadata, and final support-gate evidence remain incomplete. |
| [x] | Preview1 Node.js host link and symlink backing | `lib/src/wasi/preview1/js/web/wasi.dart`, `test/wasi_test.dart`, `test/support/node_host_fs*.dart`, `README.md` | `dart test -p node test/wasi_test.dart --name "node preview1 creates real host links and symlinks" --reporter=expanded` failed before the fix with `path_link` returning `NOENT(44)` for a real host preopen file; the same command passes after the fix. Full verification for this row: `dart test -p node test/wasi_test.dart --name "node preview1 creates real host links and symlinks" --reporter=expanded`; Node host filesystem focused tests; VM/browser Preview1 regression suites; README command/snippet tests; `dart analyze`. | Node configured host preopens now execute `path_link`, `path_symlink`, and `path_readlink` through direct Node `fs` calls after preopen-root resolution and one-level type/parent checks. Virtual files and browser JS remain on the in-memory VFS. Node host timestamp mutation, full lstat-grade symlink timestamp/set-times metadata, and final support-gate evidence remain incomplete. |
| [x] | Preview1 Node.js host symlink resolution and path metadata | `lib/src/wasi/preview1/js/web/wasi.dart`, `test/wasi_test.dart`, `test/support/node_host_fs*.dart`, `README.md` | `dart test -p node test/wasi_test.dart --name "node preview1 follows real host symlinks and reports path filestat" --reporter=expanded` failed before the fix with `path_filestat_get` returning `NOENT(44)` for a real host symlink; the same command passes after the fix. Additional focused gate: `dart test -p node test/wasi_test.dart --name "node preview1 (preopens use real host regular files|fd_readdir lists real host directories|mutates real host directory paths|creates real host links and symlinks|follows real host symlinks and reports path filestat)" --reporter=expanded`. | Node configured host preopens now resolve final real host symlinks for `path_open` and `path_filestat_get` through direct `lstatSync`/`readlinkSync`/`realpathSync` calls and reject resolved targets outside the preopen with `NOTCAPABLE`. Virtual files and browser JS remain on the in-memory VFS. Node host timestamp mutation, full lstat-grade symlink timestamp/set-times metadata, and final support-gate evidence remain incomplete. |
...
| [x] | P2/P3 resource WIT world adapter binding | `lib/src/wasi/component/wit_document.dart`, `lib/src/wasi/component/wit_adapter.dart`, `test/wasi_component_wit_test.dart`, `test/wasi_component_versioned_host_test.dart`, `tool/wasi_resource_table_benchmark.dart` | `dart test test/wasi_component_wit_test.dart --name "resource type" --reporter=expanded`; `dart test test/wasi_component_versioned_host_test.dart --name "resource handles" --reporter=expanded`; `dart test test/wasi_component_versioned_host_test.dart test/wasi_component_wit_test.dart --reporter=compact`; `dart run tool/wasi_resource_table_benchmark.dart --iterations=2000 --resources=256 --json` | Local same-interface WIT `resource` declarations now parse and resolve by name in synchronous Preview2/Preview3 adapter signatures, with owned and borrowed resource handles represented and validated as canonical `u32` values. Generated multi-package worlds, cross-interface type resolution, resource lifetime/drop integration, and async WIT adapters remain open. |
...
- Preview 1 is real but still incomplete. Native, browser JS, and Node JS hosts
  ...
  `node:wasi`, and Node now bridges configured preopened regular files to live
  host file descriptors. Native and JS hosts share
...
- Native Preview1 preopens now bridge to the real host filesystem for regular
  ...
  preopened regular files to real host file descriptors, real host directory
  entries to `fd_readdir`, basic path mutation to real host `fs` calls,
  hard-link plus symlink create/readlink calls to real host `fs` operations, and
  final symlink follow/path metadata through `path_open` and
  `path_filestat_get` when the resolved target stays inside the configured
  preopen. Node host timestamp mutation and full lstat-grade symlink
  timestamp/set-times metadata remain explicit P1 blockers. The Node
  host-filesystem
...
- Component decoding and validation exist under
  ...
  same-interface named `record`, `variant`, `enum`, `flags`, and `resource`
  handle value trees through Preview2 and Preview3 callbacks over
  `WasmComponentValueData` and canonical `u32` resource handles;
  ...
  multi-package WIT, cross-interface named type resolution, resource
  lifetime/drop integration, and async WIT execution remain future work.
...
- [ ] `WIT-INGESTION` - WIT/interface ingestion.
  ...
  - Evidence: `WIT-WORLD-VERSION-PROFILE-INGESTION` now parses annotated
    ...
    `WIT-WORLD-RESOURCE-ADAPTER-BINDING` now parses local WIT `resource`
    declarations and binds same-interface owned and borrowed resource handles
    through the same Preview2/Preview3 adapter path.
...
- [x] `WIT-WORLD-RESOURCE-ADAPTER-BINDING` - Local WIT resource handles bind to
  executable Preview2/Preview3 adapters.
  - Scope: internal WIT adapter binding for local import interface functions
    with synchronous signatures that reference resources declared in the same
    interface.
  - Edit targets: `lib/src/wasi/component/wit_document.dart`,
    `lib/src/wasi/component/wit_adapter.dart`,
    `test/wasi_component_wit_test.dart`,
  ...
  - Red test:
    `dart test test/wasi_component_wit_test.dart --name "resource type" --reporter=expanded`
    failed before the fix because local resource declarations were not
    captured;
    `dart test test/wasi_component_versioned_host_test.dart --name "resource handles" --reporter=expanded`
    failed before the fix because named resource and borrowed resource WIT
    adapter types were rejected before binding.
  - Implementation gate:
    `dart test test/wasi_component_wit_test.dart --name "resource type" --reporter=expanded`;
    `dart test test/wasi_component_versioned_host_test.dart --name "resource handles" --reporter=expanded`;
    `dart test test/wasi_component_versioned_host_test.dart test/wasi_component_wit_test.dart --reporter=compact`.
  - Performance gate:
    ...
    and
    `component_wit_resource_adapter_program_invoke.per_operation_us=0.23233333333333334`.
  - Done when: local WIT resource declarations are parsed, same-interface
    resource names resolve as owned handles and `borrow<T>` handles,
    valid handle arguments/results execute in Preview2 and Preview3 adapter
    programs, invalid handle values fail before callbacks run, and callbacks
    results are validated before returning to the caller.
  - Claim impact: advances `WIT-INGESTION`; does not complete generated
    ...
    resource lifetime/drop integration, async WIT adapter execution,
    `SUPPORT-P1`, `SUPPORT-P2`, or `SUPPORT-P3`.
...
- [x] `WIT-WORLD-VARIANT-ENUM-FLAGS-ADAPTER-BINDING` - Local WIT variant, enum,
  ...
  - Claim impact: advances `WIT-INGESTION`; does not complete generated
    multi-package world binding, cross-interface named type resolution,
    resource lifetime/drop integration, async WIT adapter execution, `SUPPORT-P1`,
    `SUPPORT-P2`, or `SUPPORT-P3`.

...
- Component decoding and validation exist under
  ...
  component values, plus canonical `u32` handles for direct resource and direct
  or memory-backed `error-context` adapter calls. It resolves callbacks by
  decoded
  ...
  rejects resource handles, nested async values, async adapters, and value
  shapes without a supported codec instead of approximating them. This keeps
  future
...
- Component resource table canonical `resource.new`/`resource.rep`/
  ...
  decoded direct error-context adapter memory invocation, decoded direct string
  adapter memory invocation, decoded core-memory primitive future-copy, and

...
- Component decoding and validation exist under
  ...
  endpoint during host-side canonical execution. Internal futures now expose a
  pending read completion primitive, and canonical async programs can await
  pending `future.read` operations through `invokeAsync`. P2/P3 host
  instantiation, WIT ingestion, full canonical ABI lowering/lifting, and full
  async stream/future execution are not
...
- Internal component stream/future endpoint round-trip, cancellation,
  completion/drop, pending future-read completion, decoded canonical async
  program invocation, and resource-table-backed borrowed handle invocation

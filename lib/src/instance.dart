...
const bool _runtimePerfStatsEnabled = bool.fromEnvironment(
  'WASD_RUNTIME_PERF',
  defaultValue: false,
);
const bool _asyncSubsetSyncFastPathEnabled = bool.fromEnvironment(
  'WASD_ASYNC_SUBSET_SYNC_FASTPATH',
  defaultValue: false,
);
...
final class _RuntimePerfStats {
  _RuntimePerfStats({required this.enabled}) {
    if (enabled) {
      _clock.start();
    }
  }
  ...
  int _invokeAsyncCalls = 0;
  int _asyncSubsetCalls = 0;
  int _asyncSubsetOpcodeCount = 0;
  int _asyncSubsetUs = 0;
  int _hostImportCalls = 0;
  int _hostImportUs = 0;
  int _hostImportAwaitCount = 0;

  int get nowUs => _clock.elapsedMicroseconds;

  void recordInvokeAsyncCall() {
    if (!enabled) {
      return;
    }
    _invokeAsyncCalls++;
    _flushIfNeeded(nowUs);
  }

  void recordHostImport({required int elapsedUs, required bool awaited}) {
    if (!enabled) {
      return;
    }
    _hostImportCalls++;
    ...
    if (awaited) {
      _hostImportAwaitCount++;
    }
    _flushIfNeeded(nowUs);
  }

  void recordAsyncSubsetExecution({
    required int elapsedUs,
    required int opcodeCount,
  }) {
    if (!enabled) {
      return;
    }
    _asyncSubsetCalls++;
    _asyncSubsetUs += elapsedUs;
    _asyncSubsetOpcodeCount += opcodeCount;
    _flushIfNeeded(nowUs);
  }

  void _flushIfNeeded(int nowUs) {
    if (nowUs - _lastFlushUs < 1000000) {
      return;
    }
    _lastFlushUs = nowUs;
    print(
      'wasd_runtime_perf '
      'invoke_async_calls=$_invokeAsyncCalls '
      'async_subset_calls=$_asyncSubsetCalls '
      'async_subset_opcodes=$_asyncSubsetOpcodeCount '
      'async_subset_us=$_asyncSubsetUs '
      'host_import_calls=$_hostImportCalls '
      'host_import_us=$_hostImportUs '
      'host_import_awaits=$_hostImportAwaitCount',
    );

    _invokeAsyncCalls = 0;
    _asyncSubsetCalls = 0;
    _asyncSubsetOpcodeCount = 0;
    _asyncSubsetUs = 0;
    _hostImportCalls = 0;
    _hostImportUs = 0;
    _hostImportAwaitCount = 0;
  }
}

final _RuntimePerfStats _runtimePerfStats = _RuntimePerfStats(
  enabled: _runtimePerfStatsEnabled,
);
...
final class WasmInstance {
  WasmInstance._({
    ...
    required List<bool> asyncSubsetMustInterpret,
       ...
       _memory64ByIndexValues = memory64ByIndex,
       _table64ByIndexValues = table64ByIndex,
       ...
       _asyncSubsetMustInterpret = asyncSubsetMustInterpret,
       _vm = WasmVm(
         functions: functions,
         types: module.types,
         tagTypes: tagTypes,
  ...
  final List<bool> _memory64ByIndexValues;
  final List<bool> _table64ByIndexValues;
  ...
  final List<bool> _asyncSubsetMustInterpret;
  final WasmVm _vm;
  ...
  factory WasmInstance.fromModule(
    ...
    final asyncOnlyHostImportByFunctionIndex = <bool>[];
    ...
    for (final import in module.imports) {
      switch (import.kind) {
        ...
        case WasmImportKind.exactFunction:
          ...
          asyncOnlyHostImportByFunctionIndex.add(
            callback == null && asyncCallback != null,
          );
    ...
    for (var i = 0; i < module.codes.length; i++) {
      ...
      asyncOnlyHostImportByFunctionIndex.add(false);
    }
    ...
    final asyncSubsetMustInterpret = _asyncSubsetSyncFastPathEnabled
        ? _computeAsyncSubsetMustInterpret(
            functions,
            asyncOnlyHostImportByFunctionIndex,
    ...
    final instance = WasmInstance._(
      ...
      hasAsyncOnlyHostImports: hasAsyncOnlyHostImports,
      asyncSubsetMustInterpret: List<bool>.unmodifiable(
        asyncSubsetMustInterpret,
      ),
    );
  ...
  static List<bool> _computeAsyncSubsetMustInterpret(
    List<RuntimeFunction> functions,
    List<bool> asyncOnlyHostImportByFunctionIndex, {
    required bool hasAsyncOnlyHostImports,
  }) {
    final functionCount = functions.length;
    final mustInterpret = List<bool>.filled(functionCount, false);
    if (!hasAsyncOnlyHostImports || functionCount == 0) {
      return mustInterpret;
    }
    ...
    void mark(int functionIndex) {
      if (functionIndex < 0 || functionIndex >= functionCount) {
        return;
      }
      if (mustInterpret[functionIndex]) {
        return;
      }
      mustInterpret[functionIndex] = true;
      pending.add(functionIndex);
    }

    final importedFunctionCount =
        asyncOnlyHostImportByFunctionIndex.length < functionCount
        ? asyncOnlyHostImportByFunctionIndex.length
        : functionCount;
    for (var i = 0; i < importedFunctionCount; i++) {
      if (asyncOnlyHostImportByFunctionIndex[i]) {
        mark(i);
      }
    }

    for (var callerIndex = 0; callerIndex < functionCount; callerIndex++) {
      final function = functions[callerIndex];
      ...
      for (final instruction in function.instructions) {
        switch (instruction.opcode) {
          ...
          case Opcodes.returnCall:
            final calleeIndex = instruction.immediate!;
            if (calleeIndex < 0 || calleeIndex >= functionCount) {
              hasDynamicCall = true;
            } else {
              callersByCallee[calleeIndex].add(callerIndex);
            }
            break;
    ...
    while (pending.isNotEmpty) {
      final calleeIndex = pending.removeLast();
      for (final callerIndex in callersByCallee[calleeIndex]) {
        mark(callerIndex);
      }
    }
  ...
  Future<Object?> invokeAsync(
    ...
    if (_runtimePerfStatsEnabled) {
      _runtimePerfStats.recordInvokeAsyncCall();
    }
    final prepared = _prepareInvoke(exportName, args);
    final function = functions[prepared.functionIndex];
    if (function is HostRuntimeFunction && function.asyncCallback != null) {
      final hostResult = await _invokeHostFunctionAsync(
        function,
        prepared.typedArgs,
      );
  ...
  Future<List<WasmValue>> _invokeFunctionAsyncSubset(
    ...
    final normalizedArgs = argsAreNormalized
        ? args
        : _normalizeArgsForType(args, function.type.params);
    ...
    if (defined.localZeroValues.isNotEmpty) {
      locals.addAll(defined.localZeroValues);
    }
    final stack = <WasmValue>[];
    final instructions = defined.instructions;
    final memory64ByIndex = _memory64ByIndexValues;
    final table64ByIndex = _table64ByIndexValues;
    final controlStack = <_AsyncSubsetControlFrame>[];
    final runtimePerfEnabled = _runtimePerfStatsEnabled;
    final subsetStartUs = runtimePerfEnabled ? _runtimePerfStats.nowUs : 0;
    var subsetOpcodeCount = 0;
    var pc = 0;
    try {
            ...
            switch (instruction.opcode) {
              ...
              case Opcodes.refFunc:
                ...
                if (targetIndex < 0 || targetIndex >= functions.length) {
                  throw RangeError(
                    'ref.func target out of range: $targetIndex',
                  );
                }
                stack.add(
                  WasmValue.i32(
              ...
              case Opcodes.refTestNullable:
                stack.add(
                  WasmValue.i32(
                    _gcRefTestAsyncSubset(stack, instruction) ? 1 : 0,
                  ),
                );
                pc++;
              ...
              case Opcodes.i8x16Shuffle:
                _simdI8x16Shuffle(
                  stack,
                  lanes: instruction.floatBytesImmediate,
                );
                pc++;
              ...
              case Opcodes.i32Rotl:
                ...
                final rotated = shift == 0
                    ? value
                    : ((value << shift) | (value >> (32 - shift))).toUnsigned(
                        32,
                      );
              ...
              case Opcodes.i64LtU:
                ...
                stack.add(
                  WasmValue.i32(
                    WasmI64.compareUnsigned(lhs.asI64(), rhs.asI64()) < 0
                        ? 1
                        : 0,
                  ),
                );
              ...
              case Opcodes.i64LeS:
                ...
                stack.add(
                  WasmValue.i32(
                    lhs.asI64().compareTo(rhs.asI64()) <= 0 ? 1 : 0,
                  ),
                );
                pc++;
              ...
              case Opcodes.f32ConvertI64U:
                ...
                stack.add(
                  WasmValue.f32(_f32FromInteger(WasmI64.unsigned(value))),
                );
                pc++;
              ...
              case Opcodes.f64CopySign:
                ...
                final magnitudeMask = BigInt.parse(
                  '7fffffffffffffff',
                  radix: 16,
                );
                stack.add(
              ...
              case Opcodes.memorySize:
                ...
                if (memoryIndex < 0 || memoryIndex >= memories.length) {
                  throw RangeError(
                    'memory.size index out of range: $memoryIndex',
                  );
                }
                final isMemory64 =
                    memoryIndex >= 0 &&
                ...
                stack.add(
                  isMemory64 ? WasmValue.i64(pages) : WasmValue.i32(pages),
                );
                pc++;
              ...
              case Opcodes.memoryGrow:
                ...
                if (memoryIndex < 0 || memoryIndex >= memories.length) {
                  throw RangeError(
                    'memory.grow index out of range: $memoryIndex',
                  );
                }
                final isMemory64 =
                    memoryIndex >= 0 &&
                ...
                stack.add(
                  isMemory64
                      ? WasmValue.i64(oldPages)
                      : WasmValue.i32(oldPages),
                );
                pc++;
              ...
              case Opcodes.memoryCopy:
                ...
                if (sourceMemoryIndex < 0 ||
                    sourceMemoryIndex >= memories.length) {
                  throw RangeError(
              ...
              case Opcodes.i32AtomicRmw16XchgU:
                final descriptor = switch (instruction.opcode) {
                  ...
                  _ => throw StateError(
                    'Unexpected i32 narrow atomic rmw opcode.',
                  ),
                };
              ...
              case Opcodes.i64AtomicRmw32XchgU:
                final descriptor = switch (instruction.opcode) {
                  ...
                  _ => throw StateError(
                    'Unexpected i64 narrow atomic rmw opcode.',
                  ),
                };
              ...
              case Opcodes.i32AtomicRmw16CmpxchgU:
                final widthBytes = switch (instruction.opcode) {
                  ...
                  _ => throw StateError(
                    'Unexpected i32 narrow cmpxchg opcode.',
                  ),
                };
              ...
              case Opcodes.i64AtomicRmw32CmpxchgU:
                final widthBytes = switch (instruction.opcode) {
                  ...
                  _ => throw StateError(
                    'Unexpected i64 narrow cmpxchg opcode.',
                  ),
                };
              ...
              case Opcodes.tableGrow:
                ...
                final previousLength = tables[tableIndex].grow(
                  delta,
                  fillValue,
                );
                stack.add(
              ...
              case Opcodes.i64Store32:
                ...
                if (memArg == null) {
                  throw StateError(
                    'Malformed memory store instruction memarg.',
                  );
                }
                final target = _resolveAsyncSubsetMemoryTarget(
                  memArg: memArg,
              ...
              case Opcodes.delegate:
                if (controlStack.isEmpty ||
                    controlStack.last.kind !=
                        _AsyncSubsetControlKind.tryLegacy) {
              ...
              case Opcodes.call:
                ...
                final callResults = await _invokeAsyncSubsetCallTarget(
                  targetIndex,
                  callArgs,
                  depth: depth + 1,
                );
              ...
              case Opcodes.callRef:
                ...
                if (typeIndex < 0 || typeIndex >= module.types.length) {
                  throw RangeError(
                    'call_ref type index out of range: $typeIndex',
                  );
                }
              ...
              case Opcodes.returnCall:
                ...
                if (targetIndex < 0 || targetIndex >= functions.length) {
                  throw RangeError(
                    'return_call target out of range: $targetIndex',
                  );
                }
                final target = functions[targetIndex];
                final callArgs = _popArgsForTypes(
                ...
                return _invokeAsyncSubsetCallTarget(
                  targetIndex,
                  callArgs,
                  depth: depth + 1,
                );
      ...
      if (runtimePerfEnabled) {
        _runtimePerfStats.recordAsyncSubsetExecution(
          elapsedUs: _runtimePerfStats.nowUs - subsetStartUs,
          opcodeCount: subsetOpcodeCount,
        );
      }
    }
  ...
  List<Object?> _externalizeArgsForHost(List<WasmValue> values) {
    switch (values.length) {
      case 0:
        return const <Object?>[];
      case 1:
        return <Object?>[values[0].toExternal()];
      case 2:
        return <Object?>[values[0].toExternal(), values[1].toExternal()];
      case 3:
        return <Object?>[
          values[0].toExternal(),
          values[1].toExternal(),
      ...
      default:
        final externalArgs = List<Object?>.filled(values.length, null);
        for (var i = 0; i < values.length; i++) {
          externalArgs[i] = values[i].toExternal();
        }
        return externalArgs;
    }
  }

  Future<Object?> _invokeHostFunctionAsync(
    HostRuntimeFunction function,
    List<WasmValue> args,
  ) async {
    final externalArgs = _externalizeArgsForHost(args);
    final runtimePerfEnabled = _runtimePerfStatsEnabled;
    final startUs = runtimePerfEnabled ? _runtimePerfStats.nowUs : 0;
    final asyncCallback = function.asyncCallback;
    var awaited = false;

    Object? hostResult;
    if (asyncCallback != null) {
      final maybeFuture = asyncCallback(externalArgs);
      if (maybeFuture is Future<Object?>) {
        awaited = true;
        hostResult = await maybeFuture;
      } else {
        hostResult = maybeFuture;
      }
    } else {
      hostResult = function.callback(externalArgs);
    }

    if (runtimePerfEnabled) {
      _runtimePerfStats.recordHostImport(
        elapsedUs: _runtimePerfStats.nowUs - startUs,
        awaited: awaited,
      );
    }
    return hostResult;
  ...
  Future<List<WasmValue>> _invokeAsyncSubsetCallTarget(
    int targetIndex,
    List<WasmValue> callArgs, {
    required int depth,
  }) async {
    if (_asyncSubsetSyncFastPathEnabled) {
      final target = functions[targetIndex];
      if (target is DefinedRuntimeFunction &&
          targetIndex < _asyncSubsetMustInterpret.length &&
          !_asyncSubsetMustInterpret[targetIndex]) {
  ...
  int _popAsyncSubsetLinearValue(
    ...
    if (expectedType == WasmValueType.i32) {
      final unsigned = rawValue.asI32().toUnsigned(32);
      if (unsigned > wasmAddressSpaceBytes) {
        throw RangeError(
          '$context exceeds supported linear range: '
    ...
    final unsigned = WasmI64.unsigned(rawValue.asI64());
    final maxSupported = BigInt.from(wasmAddressSpaceBytes);
  ...
  int _popAsyncSubsetMemoryOperationLength(
    ...
    return switch (value.type) {
      WasmValueType.i32 => () {
        final unsigned = value.asI32().toUnsigned(32);
        if (unsigned > wasmAddressSpaceBytes) {
          throw RangeError(
            '$context exceeds supported linear range: '
            '$unsigned > $wasmAddressSpaceBytes.',
      ...
      WasmValueType.i64 => () {
        final unsigned = WasmI64.unsigned(value.asI64());
        final maxSupported = BigInt.from(wasmAddressSpaceBytes);
        if (unsigned > maxSupported) {
          throw RangeError(
            '$context exceeds supported linear range: '
  ...
  int _resolveAsyncSubsetMemoryAddress({
    ...
    if (!isMemory64) {
      final base = baseAddressValue
          .castTo(WasmValueType.i32)
          .asI32()
          .toUnsigned(32);
      final address = base + memArg.offset;
      if (address > wasmAddressSpaceBytes - 1) {

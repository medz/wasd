...
- JS runtime behavior is environment-dependent: Node.js uses `node:wasi`; browsers provide a minimal `wasi_snapshot_preview1` shim for command-style flows (`proc_exit`, `args_*`, `environ_*`, `random_get`, configured-stdin `fd_read`, `fd_write`, `fd_pread`, `fd_pwrite`, `fd_tell`, `fd_allocate`, `fd_fdstat_get`, `fd_filestat_get`, `fd_filestat_set_size`, `fd_prestat_*`, `fd_close`, `clock_time_get`) and virtual filesystem basics (`fd_seek`, `fd_readdir`, `path_open`, `path_filestat_get`, writable in-memory files, path create/link/rename/unlink/remove), with explicit `ENOSYS` stubs for unsupported calls (for example `path_readlink`, `proc_raise`, `sock_*`).
- Native preview1 host support intentionally tracks the same minimal surface as the browser shim: command-style flows, configured-stdin `fd_read`, positioned file IO, file sizing/allocation, and virtual filesystem basics (`fd_seek`, `fd_readdir`, `path_open`, `path_filestat_get`, writable in-memory files, path create/link/rename/unlink/remove), while unsupported preview1 syscalls stay as explicit `ENOSYS` stubs.

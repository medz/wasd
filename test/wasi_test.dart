...
void main() {
  group('WASI', () {
    ...
    group('with instantiated module', () {
      ...
      test('start rethrows on proc_exit when returnOnExit is false', () {
      ...
      test('args_get returns inval for out-of-bounds argv buffer', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final argsGet = preview1['args_get'] as FunctionImportExportValue;
        final memory =
            (instance.exports['memory'] as MemoryImportExportValue).ref;
      ...
      test('random_get fills memory region', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
      ...
      test('random_get returns inval for out-of-bounds buffer', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final randomGet = preview1['random_get'] as FunctionImportExportValue;
        final memory =
            (instance.exports['memory'] as MemoryImportExportValue).ref;
      ...
      test('fd_write returns badf for unknown descriptors', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
      ...
      test('fd_write returns inval for out-of-bounds iovec', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final fdWrite = preview1['fd_write'] as FunctionImportExportValue;
        final memory =
            (instance.exports['memory'] as MemoryImportExportValue).ref;
      ...
      test('fd_read reports EOF with zero bytes on stdin', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
      ...
      test('fd_read returns badf for unknown descriptors', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final fdRead = preview1['fd_read'] as FunctionImportExportValue;
      ...
      test('fd_close returns badf for unknown descriptors', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
      ...
      test('fd_fdstat_get writes a character-device descriptor for stdio', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final fdFdstatGet =
            preview1['fd_fdstat_get'] as FunctionImportExportValue;
        final memory =
      ...
      test('fd_fdstat_get returns badf for unknown descriptors', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final fdFdstatGet =
            preview1['fd_fdstat_get'] as FunctionImportExportValue;
      ...
      test('clock_time_get writes a non-zero timestamp', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final clockTimeGet =
      ...
      test('clock_time_get returns inval for out-of-bounds pointer', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final clockTimeGet =
            preview1['clock_time_get'] as FunctionImportExportValue;
        final memory =
      ...
      test('clock_time_get returns inval for unsupported clock ids', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final clockTimeGet =
            preview1['clock_time_get'] as FunctionImportExportValue;
        final memory =
      ...
      test('clock_res_get writes a non-zero resolution for supported clocks', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final clockResGet =
      ...
      test('clock_res_get returns inval for unsupported clock ids', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final clockResGet =
            preview1['clock_res_get'] as FunctionImportExportValue;
        final memory =
      ...
      test('clock_res_get returns inval for out-of-bounds pointer', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final clockResGet =
            preview1['clock_res_get'] as FunctionImportExportValue;
        final memory =
      ...
      test('poll_oneoff reports host socket readiness hints', () async {
        ...
        final socketInstance = socketResult.instance;
        final preview1 = socketWasi.imports['wasi_snapshot_preview1']!;
        final pollOneoff = preview1['poll_oneoff'] as FunctionImportExportValue;
        ...
        expect(data.getUint16(outPtr + _eventErrorOffset, Endian.little), 0);
        expect(bytes[outPtr + _eventTypeOffset], _eventTypeFdRead);
        expect(_getUint64Le(data, outPtr + _eventFdReadwriteNbytesOffset), 77);
        expect(
          data.getUint16(outPtr + _eventFdReadwriteFlagsOffset, Endian.little),
          0,
        );
        ...
        expect(sockSend.ref([29, sendIovPtr, 1, 0, sendCountPtr]), _errnoAgain);
      ...
      test('poll_oneoff ignores zero-byte stream readiness hints', () async {
      ...
      test('poll_oneoff pulls host-backed stream provider readiness', () async {
      ...
      test('poll_oneoff reports queued socket accepts as readable', () async {
        final accepted = WASIPreview1Socket(receiveData: utf8.encode('client'));
        ...
        final preview1 = socketWasi.imports['wasi_snapshot_preview1']!;
        final pollOneoff = preview1['poll_oneoff'] as FunctionImportExportValue;
        final sockAccept = preview1['sock_accept'] as FunctionImportExportValue;
        final fdFdstatGet =
            preview1['fd_fdstat_get'] as FunctionImportExportValue;
        final memory =
            (socketInstance.exports['memory'] as MemoryImportExportValue).ref;
      ...
      test('positioned fd descriptor errors do not require bound memory', () {
      ...
      test('sock_recv and sock_send use host-backed stream handlers', () async {
      ...
      test('sock_recv waitall drains chunked host stream providers', () async {
        final receiveSource = utf8.encode('hello');
        var receiveOffset = 0;
        var providerCalls = 0;
        final socket = WASIPreview1Socket(
        ...
        expect(
          sockRecv.ref([34, iovPtr, 2, _riflagRecvWaitall, countPtr, flagsPtr]),
          0,
        );
      ...
      test('socket descriptor flags reject file-only flags', () async {
        final accepted = WASIPreview1Socket(receiveData: utf8.encode('client'));
        final listener = WASIPreview1Socket(pendingAccepted: [accepted]);
        ...
        expect(data.getUint32(acceptedFdPtr, Endian.little), 0xdeadbeef);
        expect(sockAccept.ref([99, _fdflagAppend, acceptedFdPtr]), _errnoBadf);
        expect(data.getUint32(acceptedFdPtr, Endian.little), 0xdeadbeef);
      ...
      test('socket syscalls return notsock for non-socket descriptors', () {
        final preview1 = wasi.imports['wasi_snapshot_preview1']!;
        final sockAccept = preview1['sock_accept'] as FunctionImportExportValue;
        final sockRecv = preview1['sock_recv'] as FunctionImportExportValue;
        final sockSend = preview1['sock_send'] as FunctionImportExportValue;
      ...
      test('datagram sockets do not expose accept rights', () async {
        final datagram = WASIPreview1Socket.datagram();
      ...
      test('sock_recv snapshots iovs before writing receive buffers', () async {
        final stream = WASIPreview1Socket(
        ...
        data.setUint32(streamIovPtr + 8, streamSecondBufferPtr, Endian.little);
        ...
        expect(
          utf8.decode(
            bytes.sublist(datagramSecondBufferPtr, datagramSecondBufferPtr + 2),
          ),
          'st',
        );
        expect(datagram.remainingReceiveMessages, isEmpty);
      ...
      test('sock_recv reports truncation for datagram sockets', () async {
        final socket = WASIPreview1Socket.datagram(
      ...
      test('sock_send rejects partial host-backed datagram writes', () async {
        final socket = WASIPreview1Socket.datagram(
          sendMessageHandler: (message) => message.length - 1,
        );
        final socketWasi = WASI(sockets: {17: socket});
      ...
      test('sock_shutdown validates how before descriptor state', () async {
        final socket = WASIPreview1Socket(receiveData: utf8.encode('in'));
      ...
      test('sock_send reports pipe after write-side shutdown', () async {
        final socket = WASIPreview1Socket();
      ...
      test(
        'path_remove_directory preserves open virtual directory fds',
        () async {
          final fileWasi = WASI(
            preopens: {'/sandbox': '/__wasd_nonexistent_preopen__'},
          );
          ...
          final data = ByteData.view(memory.buffer);
          const pathPtr = 5040;
          const openedFdPtr = 5080;
          const filestatPtr = 5104;
          const direntsPtr = 5184;
          const bufusedPtr = 5312;

          final dirPath = utf8.encode('gone');
          bytes.setAll(pathPtr, dirPath);
          expect(pathCreateDirectory.ref([3, pathPtr, dirPath.length]), 0);
          expect(
            pathOpen.ref([
          ...
          final dirFd = data.getUint32(openedFdPtr, Endian.little);
          expect(fdFilestatGet.ref([dirFd, filestatPtr]), 0);
          final openedInode = _getUint64Le(data, filestatPtr + 8);
          expect(openedInode, isNonZero);
          ...
          expect(_getUint64Le(data, filestatPtr + 8), openedInode);
          expect(_getUint64Le(data, filestatPtr + _filestatLinkCountOffset), 0);
      ...
      test('path_link creates hard links to virtual files', () async {
        ...
        expect(
          pathFilestatGet.ref([3, 0, newPathPtr, newPath.length, filestatPtr]),
          0,
        );
        expect(bytes[filestatPtr + 16], 4);

...
void main() {
  group('WasmComponent.decode', () {
    ...
    test('validates component instantiation indexes and value arguments', () {
      ...
      expect(
        missingCoreArgument.map((error) => error.message),
        contains(contains('Unknown Wasm component core memory index')),
      );
      ...
      expect(
        duplicate.map((error) => error.message),
        contains(contains('already consumed')),
      );
      ...
      expect(
        duplicateName.map((error) => error.message),
        contains(
          contains('Duplicate Wasm component instantiation argument name'),
        ),
      );

      final mismatchedImportArgument = WasmComponent.decode(
        _instantiateMismatchedImportArgumentComponentBytes(),
      ).validate();
      expect(
        mismatchedImportArgument.map((error) => error.message),
        contains(
          contains('Unknown Wasm component instantiation argument "other"'),
        ),
      );
      expect(
        mismatchedImportArgument.map((error) => error.message),
        contains(
          contains(
            'Missing Wasm component instantiation argument for import "need"',
          ),
        ),
      );

      final wrongImportSort = WasmComponent.decode(
        _instantiateWrongSortArgumentComponentBytes(),
      ).validate();
      expect(
        wrongImportSort.map((error) => error.message),
        contains(contains('has sort component but import expects function')),
      );
    ...
    test('reports component start signature mismatches', () {
      ...
      expect(
        instantiatedFunctionArgumentCount.map((error) => error.message),
        contains(contains('start argument count')),
      );
      ...
      expect(
        instantiatedValueArgumentType.map((error) => error.message),
        contains(contains('start argument type')),
      );
    });
...
Uint8List _instantiateMismatchedImportArgumentComponentBytes() =>
    Uint8List.fromList(const <int>[
      0x00,
      0x61,
      0x73,
...
Uint8List _instantiateWrongSortArgumentComponentBytes() =>
    Uint8List.fromList(const <int>[
      0x00,
      0x61,
      0x73,
